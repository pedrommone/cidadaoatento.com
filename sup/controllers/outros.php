...
class Outros extends CI_Controller {
	...
	public function opcoes() {
		...
		if ($this->form_validation->run()) {
			...
			$this->Prefeituras_model->update($dados, $this->dados['codigo']);			
			montaPagina('/outros/opcoes_sucesso', $this->dados);
		} else {
			$this->dados['dados'] = $this->Prefeituras_model->get_by_codigo($this->dados['codigo']);
	...
	public function senha() {
		...
		if ($this->form_validation->run()) {
			...
			$this->Prefeituras_model->set_senha_by_codigo($this->encrypt->sha1($senha), $codigo);
